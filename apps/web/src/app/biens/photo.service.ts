...
export type AjoutPhotosResultat =
  | { ajoutees: true; photos: Photo[] }
  | { ajoutees: false; erreurs: string[] };
...
export type SuppressionPhotoResultat =
  | { supprimee: true }
  | { supprimee: false; disparue: boolean; erreurs: string[] };
...
export class PhotoService {
  ...
  private url(bienId: number): string {
    return `/api/biens/${bienId}/photos`;
  }

  /** Les photos d'un Bien, dans l'ordre de la galerie (#13). */
  lister(bienId: number): Observable<Galerie> {
    return this.http.get<PhotoApi[]>(this.url(bienId)).pipe(
  ...
  ajouter(bienId: number, fichiers: readonly File[]): Observable<AjoutPhotosResultat> {
    ...
    return this.http.post<PhotoApi[]>(this.url(bienId), corps).pipe(
  ...
  supprimer(bienId: number, photoId: number): Observable<SuppressionPhotoResultat> {
    return this.http.delete<void>(`${this.url(bienId)}/${photoId}`).pipe(
