...
import { criteresManquants, demarrer, passer, repondre, termine, questionCourante } from './assistant';
...
describe('les Critères manquants', () => {
  it('propose tous les Critères d’un Bien qui vient d’être créé', () => {
    ...
    expect(criteresManquants(RIEN_DE_RENSEIGNE)).toHaveLength(15);
  });
  ...
  it('les propose dans l’ordre de la définition', () => {
    ...
    const ids = criteresManquants(RIEN_DE_RENSEIGNE).map(({ id }) => id);
  ...
  it('écarte les Critères déjà renseignés', () => {
    const manquants = criteresManquants({ prixDemande: 250000, dpe: 'C' });
  ...
  it('ne prend pas un Critère à zéro pour un Critère manquant', () => {
    ...
    const manquants = criteresManquants({ capaciteStationnement: 0 });
  ...
  it('ne prend pas une chaîne vide pour une réponse', () => {
    ...
    expect(criteresManquants({ adresse: '' }).map(({ id }) => id)).toContain('adresse');
  });

  it('ne propose plus rien quand tout est renseigné', () => {
    const complet = Object.fromEntries(
      criteresManquants(RIEN_DE_RENSEIGNE).map(({ id }) => [id, 'une valeur']),
    );

    expect(criteresManquants(complet)).toEqual([]);
  });
});
...
describe('le déroulé de l’assistant', () => {
  ...
  it('se termine une fois la dernière question traitée', () => {
    ...
    const presqueComplet = Object.fromEntries(
      criteresManquants(RIEN_DE_RENSEIGNE)
        .filter(({ id }) => id !== 'dpe')
