...
describe('meilleureValeurColonne', () => {
  ...
  it('ignore les Biens qui n_ont pas renseigné le Critère', () => {
  ...
  it('compare le prix au mètre carré, qui n_est pourtant pas un Critère', () => {
...
describe('lignesFaceAFace', () => {
  it('pose une ligne par Colonne, dans l_ordre du tableau', () => {
  ...
  it('porte le prix au mètre carré parmi ses lignes', () => {
    ...
    expect(lignes.some((ligne) => ligne.colonne.id === ID_COLONNE_PRIX_METRE_CARRE)).toBe(true);
  });

  it('pose une case par Bien sur chaque ligne, dans l_ordre reçu', () => {
    const ligne = ligneDe('prixDemande', [{ prixDemande: 250000 }, { prixDemande: 190000 }]);

    expect(ligne.cases.map((donnee) => donnee.texte.replace(/[  ]/g, ' '))).toEqual([
      '250 000 €',
      '190 000 €',
    ]);
  });
  ...
  it('laisse vide le prix au mètre carré d_un Bien dont la surface manque', () => {
  ...
  it('écrit les valeurs comme le tableau les écrit', () => {
    ...
    const ligne = ligneDe('surfaceHabitable', [{ surfaceHabitable: 72.5 }]);

    expect(ligne.cases[0].texte.replace(/[  ]/g, ' ')).toBe('72,5 m²');
  });
  ...
  it('dit d_une ligne si elle met quelque chose en évidence', () => {
