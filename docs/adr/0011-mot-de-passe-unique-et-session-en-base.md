...
### Amendement (#69) : un second proxy est arrivé, la décision tient, ce qui la rend vraie change

La mise en production ajoute Caddy devant nginx : il termine le TLS et relaie vers le front par le réseau interne de Compose. **Ce qui précède reste la décision** — l'en-tête est remplacé et non allongé, le comptage est par adresse, l'API n'est jointe que par le proxy. Mais deux des phrases qui la justifiaient ont cessé d'être vraies au moment où ce service est apparu, et une décision de sécurité dont la justification a vieilli est le pire état possible : la prochaine relecture partirait d'une prémisse fausse.

**« Il n'y en a pas. »** Le paragraphe sur le remplacement de l'en-tête annonçait qu'aucun proxy ne précédait nginx, et que le jour où il y en aurait un, la confiance devrait se déclarer dans `nginx.conf` plutôt que de s'hériter. Ce jour est arrivé, et c'est fait : `apps/web/nginx.conf` adopte `realip` et déclare digne de confiance **une adresse unique, celle de Caddy** (`set_real_ip_from`), que `docker-compose.prod.yml` fixe pour qu'elle soit nommable. Sans cela `$remote_addr` vaudrait l'adresse de Caddy pour tout le monde, et le « par adresse » serait devenu silencieusement le compteur global que cette décision refusait explicitement — celui que n'importe qui peut épuiser pour verrouiller le propriétaire.

Une adresse et non une plage, et c'est le détail qui décidait. Déclarer le réseau interne de Docker — `172.16.0.0/12`, la forme qui vient d'abord à l'esprit — revenait à englober la passerelle, donc l'adresse par laquelle arrive tout client direct. Mesuré pendant #69 : un `X-Forwarded-For` forgé était alors repris tel quel, là où le fichier d'avant l'écrasait. La correction supposée rétablir le comptage par adresse le retournait, et en développement, où le port du front est publié, elle ouvrait très exactement le contournement que la limitation refuse.

La nuance qui compte : `realip` corrige `$remote_addr` **avant** que les `proxy_set_header X-Forwarded-For $remote_addr` ne s'évaluent. Les deux lignes des `location` sœurs n'ont donc pas eu à changer, et ce qu'elles disent reste vrai mot pour mot — l'API ne reçoit qu'une adresse, celle que nginx a posée, la seule que le client ne choisisse pas. Seule sa provenance a changé : c'est désormais celle que Caddy a rapportée du visiteur.

La confiance est **déclarée nommément, et c'est le point de la correction**. Faire confiance à l'en-tête entrant sans dire de qui aurait marché tout autant, en s'appuyant sur le fait qu'aucun port ne mène directement à nginx. C'est vrai — `docker-compose.prod.yml` ne publie plus rien sur `web` — mais c'est une propriété d'**un autre fichier**. La protection aurait reposé sur une ligne qu'un port republié un jour pour déboguer aurait retirée, sans que rien dans `nginx.conf` n'ait bougé. La règle vit donc là où elle s'applique.

**« La liaison du port de l'API à la boucle locale est constitutive de cette protection. »** Elle l'était, et en production elle ne l'est plus : `docker-compose.prod.yml` ne publie aucun port sur `api`. L'hypothèse que le paragraphe énonçait — l'API n'est jointe que par le proxy — est donc tenue plus fermement qu'avant, par l'absence de publication plutôt que par sa restriction. La réserve qui suivait tombe du même coup **en production seulement** : le chemin direct n'est plus ouvert à qui a un accès à l'hôte, puisqu'il n'existe pas. Elle reste exacte en développement, où le port est publié sur la boucle locale et où le contournement décrit se reproduit à l'identique.

**Le schéma souffrait du même mal que l'adresse, et il est corrigé avec elle.** `X-Forwarded-Proto` était posé depuis `$scheme`, qui décrit le saut arrivant à nginx — or ce saut est en clair, Caddy ayant terminé le TLS devant. Un visiteur en HTTPS produisait donc un `http`, mesuré. Aucun consommateur ne lit cet en-tête aujourd'hui côté API — le `secure` des cookies suit `app.inProduction` et non le proxy —, si bien que rien n'en dépendait ; il est corrigé parce qu'un en-tête faux est un piège pour qui s'y fiera plus tard, et parce que cet amendement-ci prétend précisément que la chaîne dit vrai. La correction obéit à la même règle que l'adresse : ne croire que Caddy, par un `map` sur `$realip_remote_addr` — l'adresse du pair réel, `$remote_addr` ayant déjà été réécrite à ce stade.

Ce que cet amendement **ne change pas** : le carnet reste protégé par un mot de passe unique, et le TLS n'y ajoute rien. Il empêche la lecture du mot de passe en transit, ce qui est la raison pour laquelle `web` ne publie plus de port en clair — une porte contournant Caddy ferait circuler ce mot de passe en clair sur le réseau, sur un carnet qui n'en a qu'un.
